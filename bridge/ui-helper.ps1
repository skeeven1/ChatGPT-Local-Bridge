...
public static class ClbUi
{
    ...
    [StructLayout(LayoutKind.Sequential, CharSet=CharSet.Auto)] struct MONITORINFO { public int cbSize; public RECT rcMonitor; public RECT rcWork; public uint dwFlags; }
    ...
    static Dictionary<string, object> RectData(RECT rect)
    {
    ...
    public static Dictionary<string, object> WindowInfo(long hwnd)
    {
        IntPtr h = new IntPtr(hwnd);
        if (h == IntPtr.Zero || !IsWindow(h)) throw new Exception("Invalid HWND");
        RECT wr;
        ...
        POINT origin = new POINT { X = cr.Left, Y = cr.Top };
        if (!ClientToScreen(h, ref origin)) throw new Exception("ClientToScreen failed");
        cr.Right = origin.X + (cr.Right - cr.Left);
        cr.Bottom = origin.Y + (cr.Bottom - cr.Top);
        ...
        bool monitorOk = monitor != IntPtr.Zero && GetMonitorInfo(monitor, ref mi);
        uint dpi = 96;
        try { uint detected = GetDpiForWindow(h); if (detected > 0) dpi = detected; } catch { }
        var data = new Dictionary<string, object> {
            { "hwnd", hwnd }, { "pid", (long)pid },
            { "windowBounds", RectData(wr) }, { "clientBounds", RectData(cr) },
            { "visible", IsWindowVisible(h) }, { "minimized", IsIconic(h) },
            { "maximized", IsZoomed(h) }, { "active", GetForegroundWindow() == h },
            { "dpi", (int)dpi }, { "scale", dpi / 96.0 }
        };
        if (monitorOk) {
            data["monitor"] = monitor.ToInt64();
            data["monitorBounds"] = RectData(mi.rcMonitor);
            data["monitorWorkArea"] = RectData(mi.rcWork);
            data["primary"] = (mi.dwFlags & 1) != 0;
        }
        return data;
    }
    ...
    public static Rectangle FindLargestBrightRegion(out double confidence, out int foregroundX, out int foregroundY, out int foregroundWidth, out int foregroundHeight)
    {
        ...
        return FindLargestBrightRegionInScope(scope, out confidence, out foregroundX, out foregroundY, out foregroundWidth, out foregroundHeight);
    }

    public static Rectangle FindLargestBrightRegionInRect(int x, int y, int width, int height, out double confidence, out int foregroundX, out int foregroundY, out int foregroundWidth, out int foregroundHeight)
    {
        Rectangle scope = Rectangle.Intersect(VirtualScreen, new Rectangle(x, y, width, height));
        if (scope.Width < 64 || scope.Height < 64) throw new Exception("Bright-region scope is too small");
        return FindLargestBrightRegionInScope(scope, out confidence, out foregroundX, out foregroundY, out foregroundWidth, out foregroundHeight);
    }

    static Rectangle FindLargestBrightRegionInScope(Rectangle scope, out double confidence, out int foregroundX, out int foregroundY, out int foregroundWidth, out int foregroundHeight)
    {
...
while ($true) {
  ...
  try {
    ...
    switch ([string]$cmd.action) {
      ...
      'window-context' {
        $data=[ClbUi]::WindowInfo([long]$cmd.hwnd)
        Send-Reply @{ id=$id; ok=$true; data=$data }
      }
      'inspect-ui-tree' {
        $root=[System.Windows.Automation.AutomationElement]::FromHandle([IntPtr][long]$cmd.hwnd)
        ...
        $items=@()
        $limit=[Math]::Min(800,$found.Count)
        for($i=0;$i -lt $limit;$i++){
          try {
            $el=$found.Item($i);$rect=$el.Current.BoundingRectangle
            if($rect.Width -le 0 -or $rect.Height -le 0){ continue }
            $items += @{
              name=$el.Current.Name; automationId=$el.Current.AutomationId; className=$el.Current.ClassName
              controlType=$el.Current.ControlType.ProgrammaticName; enabled=[bool]$el.Current.IsEnabled; offscreen=[bool]$el.Current.IsOffscreen
              bounds=@{x=[Math]::Round($rect.X);y=[Math]::Round($rect.Y);width=[Math]::Round($rect.Width);height=[Math]::Round($rect.Height)}
            }
          } catch {}
        }
        Send-Reply @{ id=$id; ok=$true; data=@{ hwnd=[long]$cmd.hwnd; elements=$items; truncated=($found.Count -gt $limit); count=$items.Count } }
      }
      'screen-capture' {
      ...
      'find-bright-region' {
        ...
        if($null -ne $cmd.width){
          $r=[ClbUi]::FindLargestBrightRegionInRect([int]$cmd.x,[int]$cmd.y,[int]$cmd.width,[int]$cmd.height,[ref]$conf,[ref]$fx,[ref]$fy,[ref]$fw,[ref]$fh)
        } else {
